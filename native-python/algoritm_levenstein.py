...
def check_plagiarism_all(data_target_path):
    plagiarism_list = []
    for file in os.listdir('data_samples'):
        if file.endswith('.pdf'):
            user_article_ = read_pdf_file(data_target_path)
            user_article_clean_ = clean_data(user_article_)
            db_article_ = read_pdf_file(f'data_samples/{file}')
